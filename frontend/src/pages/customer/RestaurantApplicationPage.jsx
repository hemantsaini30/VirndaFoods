// src/pages/customer/RestaurantApplicationPage.jsx
...
export default function RestaurantApplicationPage() {
  ...
  const [form, setForm] = useState({ name: '', address: '', city: '', documentsUrl: '' });
  ...
  async function handleSubmit(e) {
    ...
    try {
      const data = await submitRestaurantApplication({
        name: form.name,
      ...
      <div className="mx-auto max-w-lg px-4 py-12 sm:px-6 lg:px-8">
        <div className="rounded-xl bg-white p-6 shadow-sm sm:p-8">
          <div className="flex items-center justify-between">
            <h1 className="text-xl font-bold text-gray-900">Your restaurant application</h1>
            <StatusBadge status={application.status} />
          </div>
          <dl className="mt-4 space-y-2 text-sm text-gray-700">
            <div>
              <dt className="font-medium text-gray-500">Restaurant name</dt>
          ...
          {application.status === 'APPROVED' && (
            <p className="mt-4 text-sm text-green-700">
              Congratulations — your restaurant is now live on the platform!
            </p>
          )}
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-12 sm:px-6 lg:px-8">
      <div className="rounded-xl bg-white p-6 shadow-sm sm:p-8">
        <h1 className="text-xl font-bold text-gray-900">Apply to list your restaurant</h1>
        <p className="mt-1 text-sm text-gray-600">
          Tell us about your restaurant. An admin will review your application.
        </p>
        ...
        <form className="mt-6 space-y-5" onSubmit={handleSubmit} noValidate>
          ...
          <div>
            <label htmlFor="documentsUrl" className="block text-sm font-medium text-gray-700">
              Supporting documents URL <span className="text-gray-400">(optional)</span>
            </label>
            <input
              id="documentsUrl"
              name="documentsUrl"
