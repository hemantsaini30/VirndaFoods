// tests/deliveryPartnerApplications.test.js
...
function submitApplication(accessToken, overrides = {}) {
  return request(app)
    .post('/api/v1/delivery-partner-applications')
...
// Same eventual-consistency caveat as restaurantApplications.test.js —
// the notifications listener isn't awaited by the emitting service.
...
describe('POST /api/v1/delivery-partner-applications', () => {
  it('lets a CUSTOMER submit an application, starting PENDING', async () => {
    ...
    expect(res.body.application.vehicleType).toBe('Bike');
  });

  it('lets a RESTAURANT_OWNER submit an application too', async () => {
    const { accessToken } = await registerAndLogin({ role: 'RESTAURANT_OWNER' });
  ...
  it('blocks an existing DELIVERY_PARTNER from submitting with 403', async () => {
    const { accessToken } = await registerAndLogin({ role: 'DELIVERY_PARTNER' });
  ...
  it('rejects a second active application with 409', async () => {
    ...
    const second = await submitApplication(accessToken, { vehicleType: 'Scooter' });
...
describe('GET /api/v1/delivery-partner-applications/me', () => {
  it("returns the applicant's own latest application", async () => {
    const { accessToken } = await registerAndLogin({ role: 'CUSTOMER' });
    await submitApplication(accessToken);

    const res = await request(app)
      .get('/api/v1/delivery-partner-applications/me')
      .set('Authorization', `Bearer ${accessToken}`);
...
describe('GET /api/v1/delivery-partner-applications (admin list)', () => {
  it('blocks a non-admin with 403', async () => {
    const { accessToken } = await registerAndLogin({ role: 'CUSTOMER' });

    const res = await request(app)
      .get('/api/v1/delivery-partner-applications')
      .set('Authorization', `Bearer ${accessToken}`);
  ...
  it('lets an admin list applications, filtered by status', async () => {
    ...
    const res = await request(app)
      .get('/api/v1/delivery-partner-applications?status=PENDING')
...
describe('PATCH /api/v1/delivery-partner-applications/:id/status', () => {
  it('blocks a non-admin with 403', async () => {
    const { accessToken: customerToken } = await registerAndLogin({ role: 'CUSTOMER' });
    const submitRes = await submitApplication(customerToken);

    const res = await request(app)
      .patch(`/api/v1/delivery-partner-applications/${submitRes.body.application.id}/status`)
  ...
  it('allows the PENDING -> REJECTED shortcut', async () => {
    ...
    const res = await request(app)
      .patch(`/api/v1/delivery-partner-applications/${submitRes.body.application.id}/status`)
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ status: 'REJECTED', reviewNote: 'Missing license.' });
  ...
  it('walks the full PENDING -> UNDER_REVIEW -> APPROVED path (no linked live record, status-only)', async () => {
    ...
    const underReviewRes = await request(app)
      .patch(`/api/v1/delivery-partner-applications/${applicationId}/status`)
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ status: 'UNDER_REVIEW' });
